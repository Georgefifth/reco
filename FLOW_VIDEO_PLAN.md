...
> ReCo connects daily symptoms, cognitive observations, return-to-learn, and return-to-play. A judge can explore a realistic recovery locally with one click.
...
> Instead of presenting 22 symptoms in one overwhelming page, ReCo groups them into four short sections. Each symptom uses one clear zero-to-six scale. Daily context helps users notice possible relationships without claiming cause or diagnosis.
...
> Transparent local calculations look for relationships among symptoms, sleep, screen time, and light activity. ReCo labels them as discussion points—not medical conclusions.
...
> Three short browser tasks help users observe personal changes in reaction, working memory, and attention. They are wellness observations and never provide medical clearance.
...
> Sport and learning recover on separate timelines. ReCo follows an international-consensus return-to-sport progression and adds a four-stage return-to-learn path with temporary, symptom-matched school supports.
...
> Journal inference runs through Ollama on the user’s own computer. Before inference, deterministic rules intercept emergency language and remove basic personal identifiers. After inference, ReCo checks the response for unsafe medical claims. The audit log stores outcomes, never journal text.
...
> Users can export or delete everything. A printable recovery summary supports conversations with clinicians. Every major feature maps to public guidance, and the app states what it cannot do.
...
> Render serves the application, but the health record stays in IndexedDB. Even AI inference remains on localhost.
